...
export default function SummonDayScreen() {
  ...
  const [summonCompareScore, setSummonCompareScore] = useState({
    currentScore: 0,
    targetScore: '',
    difference: 0,
    pointsNeeded: 0,
  ...
  const dragons: Record<string, DragonConfig> = {
    ...
    legendary: {
      name: t('summonDay.legendaryDragon'),
  ...
  const [resources, setResources] = useState<Record<string, ResourceScore>>({
    lightRegent: {
      name: t('summonDay.lightRegent'),
      value: '',
      pointsPerUnit: 70,
      batchSize: 1,
      image: 'https://iili.io/doRwllR.png',
    },
    forgeHammers: {
      name: t('summonDay.forgeHammers'),
      value: '',
      pointsPerUnit: 100,
      batchSize: 1,
    ...
    galleryShards: {
      name: t('summonDay.galleryShards'),
      value: '',
      pointsPerUnit: 1000,
      batchSize: 1,
  ...
  const loadSummonCompareScore = async () => {
    try {
      const weekNumber = getWeekNumber();
      const storageKey = `scores_week_${weekNumber}`;
      const savedData = await AsyncStorage.getItem(storageKey);
      ...
      if (savedData) {
        ...
        setSummonCompareScore(prev => ({
          ...prev,
          currentScore: parsedData.summon_day_score || 0,
        }));
      }
    } catch (error) {
      console.error('Error loading summon score:', error);
    }
  };

  const calculateSummonComparison = (targetValue: string) => {
    const target = parseInt(targetValue) || 0;
    const difference = target - summonCompareScore.currentScore;
  ...
  // Add function to calculate resource total
  ...
  // Dragon level calculation system (same as Dragon Day)
  ...
  // Dragon scoring system (same as Dragon Day)
  const calculateDragonScore = (dragonType: string, dragon: DragonLevel) => {
    ...
    switch (dragonType) {
      ...
      case 'legendary': {
        // Legendary dragons use simple multiplication
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Update with current Summon Day score
      const updatedScores = {
        ...existingScores,
        summon_day_score: total,
        summon_day_starting_score: parseInt(startingScore) || 0,
      ...
      Alert.alert(t('common.success'), t('summonDay.scoreSaved'));
      ...
      Alert.alert(t('common.error'), t('summonDay.saveError'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderDragonCard = (dragonType: string, config: DragonConfig) => {
    ...
    // Updated stats display with multiple level-ups (same as Dragon Day)
    let stats = null;
      ...
      <View style={styles.overlay}>
        ...
        >
          <View style={styles.content}>
            ...
            {/* Starting Score Section - Fixed translation keys */}
            ...
            <View style={styles.totalContainer}>
              ...
              <View style={styles.totalRow}>
                <Text style={styles.totalLabel}>Total Summon Day Score</Text>
            ...
            <View style={styles.buttonContainer}>
              ...
              >
                <Text style={styles.inlineButtonText}>
                  {loading ? t('common.saving') : t('summonDay.saveScore')}
                </Text>
              </TouchableOpacity>
            ...
            {savedScores && (
              <View style={styles.savedScoresContainer}>
                <Text style={styles.savedScoresTitle}>📱 {t('summonDay.savedOnDevice')}:</Text>
                <Text style={styles.savedScoresText}>
                  Starting Score: {savedScores.summon_day_starting_score || 0}
                </Text>
                <Text style={styles.savedScoresText}>
                  {t('summonDay.summonDayScore')}: {savedScores.summon_day_score || 0}
                </Text>
                <Text style={styles.savedScoresText}>
                  {t('summonDay.saved')}: {savedScores.last_updated ? new Date(savedScores.last_updated).toLocaleString() : t('summonDay.never')}
                </Text>
                <Text style={styles.savedScoresText}>
                  {t('summonDay.week')}: {getWeekNumber()}
                </Text>
              </View>
            )}
            ...
            <View style={styles.infoContainer}>
              <Text style={styles.infoText}>
                💾 {t('summonDay.scoresLocalInfo')}
              </Text>
              <Text style={styles.infoText}>
                🔄 {t('summonDay.dataResetInfo')}
              </Text>
            </View>
          </View>
        </ScrollView>
        ...
        {showCompareOverlay && (
          ...
          >
            <View style={compareStyles.overlayContent}>
              <Text style={compareStyles.overlayTitle}>Summon Day Quick Compare</Text>
              
              <View style={[
                compareStyles.summonComponent,
                summonCompareScore.targetScore.length > 0 && compareStyles.summonComponentWithTarget
              ]}>
                <Text style={compareStyles.summonTitle}>Summon Day</Text>
                
                <View style={compareStyles.scoreRow}>
                  <Text style={compareStyles.scoreLabel}>Current Score:</Text>
                  <Text style={compareStyles.currentScore}>{formatNumber(summonCompareScore.currentScore)}</Text>
                </View>
                ...
                <View style={compareStyles.inputContainer}>
                  ...
                  <TextInput
                    ...
                    keyboardType="numeric"
                    value={summonCompareScore.targetScore}
                    onChangeText={(value) => calculateSummonComparison(value.replace(/[^0-9]/g, ''))}
                    placeholder="0"
                    placeholderTextColor="#666"
                  />
                </View>

                {summonCompareScore.targetScore.length > 0 && (
                  <View style={compareStyles.resultsContainer}>
                    <View style={compareStyles.resultRow}>
                      ...
                      <Text style={[
                        ...
                        summonCompareScore.difference >= 0 ? compareStyles.positiveValue : compareStyles.negativeValue
                      ]}>
                        {summonCompareScore.difference >= 0 ? '+' : ''}{formatNumber(summonCompareScore.difference)}
                      </Text>
                    </View>
                    
                    {summonCompareScore.pointsNeeded > 0 ? (
                      <View style={compareStyles.resultRow}>
                        <Text style={compareStyles.needLabel}>Points Needed:</Text>
                        <Text style={compareStyles.needValue}>{formatNumber(summonCompareScore.pointsNeeded)}</Text>
                      </View>
                    ) : (
...
const styles = StyleSheet.create({
  ...
  // Starting Score Styles - Copied from Unit Day
  ...
  sectionBarCompleted: {
    backgroundColor: 'rgba(77, 148, 255, 0.8)',
    borderColor: 'rgba(102, 163, 255, 0.7)',
  },
  sectionBarText: {
    color: '#ffffff',
    fontSize: 16,
  ...
  pointsText: {
    color: '#ffffff',  // White text instead of blue
    fontWeight: 'bold',
    fontSize: 14,
    fontFamily: 'MedievalSharp',
    textShadowColor: 'rgba(0, 0, 0, 0.8)',
  ...
  pointsBadge: {
    backgroundColor: 'rgba(77, 148, 255, 0.8)',  // Changed to blue background
    paddingHorizontal: 8,
    paddingVertical: 4,
    borderRadius: 8,
    ...
    marginTop: 4,
    borderWidth: 1,
    borderColor: 'rgba(77, 148, 255, 0.9)',  // Changed border to match blue theme
  },
  pointsBadgeText: {
  ...
  totalContainer: {
    ...
    marginTop: 20,
    borderWidth: 2,
    borderColor: 'rgba(77, 148, 255, 0.5)',
  },
  totalBreakdown: {
    marginBottom: 8,
  },
  ...
  viewButton: {
    backgroundColor: 'rgba(255, 150, 77, 0.8)',
    borderColor: 'rgba(255, 150, 77, 0.9)',
  },
